  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Creador 🧸;;;\nFN:Creador 🧸\nORG:Creador 🧸\nTITLE:\nitem1.TEL;waid=51917553778:+51 997 467 668\nitem1.X-ABLabel:Creador 🧸Creador 🧸\nX-WA-BIZ-DESCRIPTION:ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:Creador 🧸\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: '𓃓𝑪𝑰𝑵𝑻𝑯𝒀𝑰𝑨♡ ✨', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  const msg = {
    ...
    unreg: '*¡Para Continuar Con Esta Función Es Necesario Registrarse!*\n\n!verificar nombre.edad\n\n*Uso Correcto* : !verificar Gato.17',
    restrict: '*!Esta Función Fué Deshabilitado Por Mi Desarrollador*',
    }[type];
  const aa = {quoted: m, userJid: conn.user.jid};
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '👋🏻 !𝗛𝗼𝗹𝗮!', body: '🍭 𝙻𝙸𝙽𝙳𝙰 𝙼𝙰𝙽̃𝙰𝙽𝙰 🍭', thumbnail: imagen1, sourceUrl: 'https://whatsapp.com/channel/0029VaCRoNYDuMRkzqWjG80Z'}}}}, aa);
